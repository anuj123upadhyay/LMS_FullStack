...
export const buySubscription = asyncHandler(async(req,res,next)=>{
     ...
     const subscription = await razorpay.subscriptions.create({
        ...
        customer_notify:1, // 1 means razorpay will handle notifying the customer , 0 means we will not notifying the customer
...
export const verifySubscription = asyncHandler(async(req,res,next)=>{
    ...
    //generating a signature with SHA@%^ for a verification purposes
  ...
  // check if generated signature and signature reeived from the frontend is the same or not
  if(generatedSignature !== razorpay_signature){
    return next(new AppError('Payment not verified ,please try again.', 400))
  }


  //if they matched created payment and store it in the DB
  ...
  //// upadet the user subscription status to active
...
//creating a subscription using razorpay that we imported from the server
    try {
        ...
        // adding the subscription status o the user account
        user.subscription.status = subscription.status;
        await user.save()
    } catch (error) {
