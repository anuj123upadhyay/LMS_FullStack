    ...
    <Layout>
        <div className='min-h-[90vh] flex items-center justify-center'>
            <div className='my-10 flex flex-col gap-4 rounded-lg p-4 text-white w-80 shadow-[0_0_10px_black]'>
                <img src={userData?.avatar?.secure_url} className='w-40 m-auto rounded-full border border-black' alt="user profile image" 
                />
        ...
        {/* button to chnage the password */}
        <div className='flex items-center justify-center ustify-between gap-2'>
            <Link 
            to={userData?.email === "test@gmail.com" ? "/denied" :"/changepassword"}
            className='w-1/2 bg-yellow-600 hover:bg-yellow-900
            transition-all
            easse-in-out
            duration-300
            rounded-sm py-2
        ...
        className='w-1/2
        border border-yellow-600 hover:border-yellow-400 transition-all ease-in-out duration-300 rounded-sm py-2 font-semibold cursor-pointer text-center'
            >
